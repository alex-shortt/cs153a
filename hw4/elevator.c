...
void QHsmTst_ctor(void) {
	...
	HSM_QHsmTst.emergency = 0;
	HSM_QHsmTst.emergency_total_time = 0;
	HSM_QHsmTst.emergency_count = 0;
	for (x=0; x<5; x++){
...
QState QHsmTst_stopped(QHsmTst *me) {
	switch (Q_SIG(me)) {
		case Q_ENTRY_SIG: {
        	...
        	case TICK_SIG: {
	    		...
	    		if (checkPending() == -1){ /*If the current floor is pending, stop for STOP_TIME_F*/
					...
					if(HSM_QHsmTst.emergency && HSM_QHsmTst.curr_floor == 0) {
							HSM_QHsmTst.emergency = 0;
							HSM_QHsmTst.emergency_total_time += simTime - HSM_QHsmTst.emergency_start;
							clearPending();
					}
					else if (HSM_QHsmTst.stop_time < STOP_TIME_F-1) HSM_QHsmTst.stop_time++;
		    		else {
			    		HSM_QHsmTst.stop_time = 0;
		...
		case EMER_SIG: {
			...
			HSM_QHsmTst.emergency = 1;
			HSM_QHsmTst.emergency_count++;
			HSM_QHsmTst.emergency_start = simTime;
			clearPending();
	...
	switch (Q_SIG(me)) {
		case Q_ENTRY_SIG: {
        	...
        	case TICK_SIG: {
				...
				if (checkPending() == -1) { 
					STOP_TIME_F = 5 + (rand() % 10) + 1;
					return Q_TRAN(&QHsmTst_stopped);
				 } /*Switch to stopped if the current floor is pending*/ 
...
void clearPending(){
	for (int x=0; x<5; x++){
		HSM_QHsmTst.floor_req_curr[x]=0;
		HSM_QHsmTst.floor_pen[x]=0;
		HSM_QHsmTst.floor_curr_call_time[x] = 0;
		}
}
...
void printData(void){
	...
	printf("Emergency average time: %f\n", HSM_QHsmTst.emergency_total_time / HSM_QHsmTst.emergency_count);
	printf("\n");
	return;
}
